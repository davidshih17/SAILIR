...
# ── Upstream enumeration: candidates from the STORE alone, no cu ──────────
# Enumeration today walks the whole cu -- one materialised, substituted
# equation per (store key x op x shift) -- and it does that every step for
# every beam state, after _aux_to_result has rebuilt that same list entry by
# entry. Both costs grow with depth (iraws climbs ~230/step).
#
# The set it selects can be characterised without cu at all. A candidate
# (op, seed) survives Phase 1a/1b exactly when its raw equation contains some
# term T that is either the target itself (1a) or a store key whose solution
# contains the target (1b). So with
#
#     S = {target} U {K in store : target in sol(K)}
#
# every candidate is seed = T - shift for some T in S, i.e. delta =
# (T - target) - shift. That is |S| x 332 integer operations and nothing is
# built. Cost scales with |S|, not with |store| x 332.
#
# This is MEASUREMENT ONLY for now: SAILIR_TC_UPENUM_DIFF=1 computes the set
# alongside the real enumeration and reports disagreement. It does not change
# what the search does. `extra` (generated but not enumerated) is EXPECTED --
# the subsector filter and the cancellation check remove those downstream.
# `missing` (enumerated but not generated) would be fatal and is the number
# that decides whether this reformulation is usable.
_TC_UPENUM_DIFF = bool(os.environ.get('SAILIR_TC_UPENUM_DIFF'))
# REPLACEMENT mode: cu is never built, unpacked, scanned or extended -- the
# store-only rule supplies the action list outright. aux_flat simply stays
# None, which every consumer already treats as "no cache, recompute", so the
# one equation needed to APPLY the chosen action is rebuilt on demand.
_TC_UPENUM_REPLACE = os.environ.get(
    'SAILIR_TC_UPENUM_REPLACE') not in (None, '', '0')
_TC_UPENUM_EVERY = int(os.environ.get('SAILIR_TC_UPENUM_EVERY', '25'))
_TC_UPENUM_STATS = {'steps': 0, 'missing': 0, 'extra': 0, 'nS': 0,
                    'n_true': 0, 'n_cand': 0, 'n_store': 0, 'worst': None}


# ── weight, without the waste (ported from beam_search_v9) ────────────────
# ibp_env.weight() makes THREE passes over the indices and allocates a 15-tuple
# of absolute values every call; callers that want only (w1, w2) discard it.
# cProfile at beam 40: 4.3M calls, ~59s of a ~110s run -- two thirds of every
# step. One pass, two scalars, no allocation, memoised on the integral.
_W12_CACHE = {}
_W12_CAP = 2_000_000
...
def _tc_upenum_candidates(target, resolved_subs, env, reg, n_idx):
    ...
    for op, (sh, vi) in vec.items():
        ...
        terms = tbl[op]
        if surv.shape[0]:
            # ONE matmul for every survivor: (n_surv, n_idx) x (n_idx, m)
            L = lin[op]
            RC = (surv.astype(np.int64) @ L[:, :n_idx].T + L[:, n_idx]) \
                % ibp_env.PRIME
            RC_l = RC.tolist()
            for si in range(surv.shape[0]):
                seed = tuple(int(x) for x in surv[si])
                if _tc_target_survives(seed, terms, resolved_subs, target,
                                       tid, n_idx, RC_l[si]):
                    kept.append((op, seed))
                else:
                    n_bad += 1

    _TC_WHY['sec_culled'] = _TC_WHY.get('sec_culled', 0) + n_bad
...
_TC_UPENUM_WHY = bool(os.environ.get('SAILIR_TC_UPENUM_WHY'))
_TC_WHY = {'no_coeff': 0, 'stripped': 0, 'cancelled': 0, 'sector': 0,
           'unexplained': 0, 'checked': 0, 'pass_bad': 0, 'pass_subs': 0,
           'sec_raw': 0, 'sec_subs': 0, 'sec_both': 0, 'sec_neither': 0,
           'sec_culled': 0}


def _tc_upenum_why(target, extra, state, env):
    """Classify each generated-but-not-enumerated candidate by WHICH downstream
    test killed it. Mirrors packed_cu.enumerate_...'s Phase 1a exactly:

      no_coeff   the identity's coefficient polynomial vanished at this seed,
                 so the term is simply absent from the raw equation
      stripped   the term is below the start weight (a passenger) and the
                 cached-raw strip removed it -- should be ZERO for S members
      cancelled  target present in the raw but gone (or zero) after the store
                 substitution
      sector     survives, but the equation drags in integrals outside the
                 target's family -- the subsector filter rejects it
      unexplained passed every test, so enumeration should have returned it;
                 non-zero here means the classifier disagrees with enumeration

    EXPENSIVE (materialises every rejected candidate). Measurement runs only.
    """
    from sailir.ibp_env import (apply_resolved_subs, cached_union_bitmask,
                                sector_bitmask)
    from sailir.packed_rs_ops import apply_resolved_subs_dict_x_packed
    rs = state.resolved_subs
    packed = bool(rs) and hasattr(next(iter(rs.values())), 'ids')
    tbm = sector_bitmask(target)
    not_tbm = ~tbm
    thr = ibp_env.get_raw_strip_threshold()
    n_idx = ibp_env.N_INDICES
    w = ibp_env.weight
    for op, delta in extra:
        ...
        # MUST substitute before judging. `target in raw` is the DIRECT rule
        # only -- an indirect action is legal exactly because the target
        # appears after the store is substituted in, so for those the target
        # is SUPPOSED to be missing from the raw equation. Testing raw
        # membership first mislabels every indirect candidate.
        cached = (apply_resolved_subs_dict_x_packed(
                      raw, rs, _V7_REGISTRY, ibp_env.PRIME)
                  if packed else apply_resolved_subs(raw, rs))
        # Both helpers return a DICT (Phase 1a consumes it as one).
        if target not in cached or cached[target] == 0:
            if in_raw:
                # Present before substitution, gone after: a dependent
                # relation (or a coefficient that hit 0 mod PRIME).
                _TC_WHY['cancelled'] += 1
            elif thr is not None and w(target)[:2] < tuple(thr):
                _TC_WHY['stripped'] += 1
            else:
                # The equation never involves the target at all -- neither
                # directly nor through any substitution in it.
                _TC_WHY['no_coeff'] += 1
            continue
        if (cached_union_bitmask(cached) & not_tbm) != 0:
            _TC_WHY['sector'] += 1
            # WHERE do the banned bits come from? After substitution the
            # equation is (raw terms that are NOT store keys) together with
            # (the solutions of the raw terms that ARE store keys). If the
            # offending bits are already in the first group, the candidate is
            # rejectable from seed + shift arithmetic alone -- no store, no
            # substitution, zero cost. If they only appear in the second, we
            # need a per-substitution sector mask (and that bound can
            # over-reject).
            b_raw = 0
            b_sub = 0
            for k in raw:
                v = rs.get(k)
                if v is None:
                    b_raw |= sector_bitmask(k)
                else:
                    ids = getattr(v, 'ids', None)
                    if ids is None:
                        for t in v:
                            b_sub |= sector_bitmask(t)
                    else:
                        for i in ids:
                            b_sub |= sector_bitmask(
                                _V7_REGISTRY.get_tuple(int(i)))
            bad_raw = (b_raw & not_tbm) != 0
            bad_sub = (b_sub & not_tbm) != 0
            if bad_raw and bad_sub:
                _TC_WHY['sec_both'] += 1
            elif bad_raw:
                _TC_WHY['sec_raw'] += 1
            elif bad_sub:
                ...
                _TC_WHY['sec_neither'] += 1
        else:
            _TC_WHY['unexplained'] += 1


def _tc_store_passenger_audit(state):
    """Are the resolved substitutions really 100% free of passengers (terms
    below the start weight)? Counts store entries containing any such term."""
    thr = ibp_env.get_raw_strip_threshold()
    if thr is None:
        if not _TC_WHY.get('thr_shown'):
            _TC_WHY['thr_shown'] = 1
            print('[upenum-why] raw strip threshold is None -- stripping is '
                  'DISABLED in this run, so nothing is expected to be '
                  'stripped', flush=True)
        return
    # The threshold is the FULL weight key (w1, w2, |abs| tuple), not a pair.
    # Comparing a 2-tuple against it makes Python call the shorter tuple
    # smaller on a leading tie, so every term sitting exactly AT the start
    # weight was scored a passenger. Compare (w1, w2) against (w1, w2).
    thr = tuple(thr)[:2]
    reg = _V7_REGISTRY
    w = ibp_env.weight
    show = not _TC_WHY.get('thr_shown')
    if show:
        _TC_WHY['thr_shown'] = 1
        print(f'[upenum-why] raw strip threshold (w1,w2) = {thr}; '
              f'SECTOR_RANK={_SECTOR_RANK} '
              f'START_SECTOR={_START_SECTOR} '
              f'STRIP_TOTAL={_STRIP_TOTAL}', flush=True)
    shown = 0
    for _k, sol in state.resolved_subs.items():
        _TC_WHY['pass_subs'] += 1
        ids = getattr(sol, 'ids', None)
        terms = ((reg.get_tuple(int(i)) for i in ids) if ids is not None
                 else iter(sol))
        for t in terms:
            if w(t)[:2] < thr:
                _TC_WHY['pass_bad'] += 1
                # Print the actual offender once or twice: which store entry,
                # which term, its weight, and whether the full is_active test
                # (sector + weight + total order) also calls it a passenger.
                if show and shown < 4:
                    shown += 1
                    print(f'[upenum-why]   store key {list(_k)} '
                          f'-> term {list(t)} weight={w(t)[:2]} '
                          f'is_active={is_active(t, thr)}', flush=True)
                break


_TC_DUMP_AT = os.environ.get('SAILIR_TC_DUMP_AT')
_TC_DUMP_TO = os.environ.get('SAILIR_TC_DUMP_TO')


def _tc_dump_state(target, state):
    """Pickle (expr, resolved_subs, target) at a chosen depth and stop.

    Exists so the enumeration paths can be benchmarked against a REAL deep
    state in seconds, instead of waiting minutes for a search to reach that
    depth every time the generator is edited. Dump once per depth, then
    iterate against the file.
    """
    if not _TC_DUMP_AT or not _TC_DUMP_TO:
        return
    # Comma-list of depths: capture them all in ONE replay instead of paying a
    # fresh replay per depth. _TC_DUMP_TO is a prefix; files get -<depth>.pkl.
    wanted = [int(x) for x in _TC_DUMP_AT.split(',')]
    depth = len(state.path)
    if depth not in wanted:
        return
    import pickle as _pk
    with open(f'{_TC_DUMP_TO}-{depth}.pkl', 'wb') as fh:
        # Materialise to PLAIN DICTS. PackedEq holds integral IDs from THIS
        # process's registry; pickling them and loading into a fresh registry
        # would silently remap every integral.
        _pk.dump({'expr': dict(state.expr),
                  'resolved_subs': {tuple(k): dict(v) for k, v
                                    in _rs_as_dict(state.resolved_subs).items()},
                  'target': tuple(target),
                  'depth': len(state.path),
                  # Record the strip context AS SET, rather than letting the
                  # bench reconstruct it: production derives both from the
                  # START integral, not the current target, and getting that
                  # wrong changes which terms the old path strips.
                  'strip_thr': ibp_env.get_raw_strip_threshold(),
                  'strip_cone': ibp_env._RAW_STRIP_CONE,
                  'sector_rank': _SECTOR_RANK,
                  'start_sector': _START_SECTOR,
                  'strip_total': _STRIP_TOTAL,
                  'start_total_key': _START_TOTAL_KEY}, fh, protocol=4)
    print(f'[dump] wrote depth={depth} '
          f'store={len(state.resolved_subs)} -> {_TC_DUMP_TO}-{depth}.pkl',
          flush=True)
    if depth >= max(wanted):
        raise SystemExit(0)
...
def _tc_upenum_valid(target, state, env):
    """Enumeration replacement. Sorted for a deterministic order; the cull
    re-sorts by (bound, op, delta) anyway, so input order cannot change the
    ranked output."""
    _tc_dump_state(target, state)
    cand, _ns = _tc_upenum_candidates(target, state.resolved_subs, env,
                                      _V7_REGISTRY, ibp_env.N_INDICES)
    return sorted(cand)


def _tc_upenum_check(state, target, valid, env):
    """Compare the store-only candidate set against what enumeration returned."""
    if not _TC_UPENUM_DIFF:
        return
    rs = getattr(state, 'resolved_subs', None)
    if rs is None:
        return
    t0 = time.time()
    cand, n_s = _tc_upenum_candidates(target, rs, env, _V7_REGISTRY,
                                      ibp_env.N_INDICES)
    dt = time.time() - t0
    true_set = {(o, tuple(d)) for o, d in valid}
    missing = true_set - cand
    st = _TC_UPENUM_STATS
    st['steps'] += 1
    st['nS'] += n_s
    st['n_true'] += len(true_set)
    st['n_cand'] += len(cand)
    st['n_store'] += len(rs)
    st['extra'] += len(cand - true_set)
    if _TC_UPENUM_WHY:
        _tc_upenum_why(target, cand - true_set, state, env)
        _tc_store_passenger_audit(state)
    if missing:
        st['missing'] += len(missing)
        if st['worst'] is None:
            st['worst'] = (len(state.path), len(missing),
                           sorted(missing)[:3])
        print(f'[upenum] MISSING at depth {len(state.path)}: '
              f'{len(missing)} of {len(true_set)} enumerated actions were '
              f'NOT generated from the store', flush=True)
    if st['steps'] % _TC_UPENUM_EVERY == 0:
        n = st['steps']
        print(f'[upenum] steps={n} store~{st["n_store"] // n} '
              f'|S|~{st["nS"] // n} generated~{st["n_cand"] // n} '
              f'enumerated~{st["n_true"] // n} '
              f'missing={st["missing"]} extra~{st["extra"] // n} '
              f'gen_time={dt * 1000:.1f}ms', flush=True)
        if _TC_UPENUM_WHY:
            w = _TC_WHY
            c = max(1, w['checked'])
            print(f'[upenum-why] of {w["checked"]} rejected candidates: '
                  f'sector={w["sector"]} ({100.0 * w["sector"] / c:.1f}%) '
                  f'absent={w["no_coeff"]} '
                  f'({100.0 * w["no_coeff"] / c:.1f}%) '
                  f'cancelled={w["cancelled"]} '
                  f'({100.0 * w["cancelled"] / c:.1f}%) '
                  f'stripped={w["stripped"]} '
                  f'unexplained={w["unexplained"]}', flush=True)
            sc = max(1, w['sector'])
            print(f'[upenum-why] banned-integral source: '
                  f'raw_only={w["sec_raw"]} '
                  f'({100.0 * w["sec_raw"] / sc:.1f}%) '
                  f'subs_only={w["sec_subs"]} '
                  f'({100.0 * w["sec_subs"] / sc:.1f}%) '
                  f'both={w["sec_both"]} '
                  f'({100.0 * w["sec_both"] / sc:.1f}%) '
                  f'neither={w["sec_neither"]}', flush=True)
            print(f'[upenum-why] store passenger audit: '
                  f'{w["pass_bad"]}/{w["pass_subs"]} substitutions contain a '
                  f'term below the start weight', flush=True)
...
def _tc_subs_json(state):
    """JSON-able view of state.resolved_subs in the SAME shape the recorder
    writes its `subs`: [[sub_int, [[integral, coeff], ...]], ...].

    Only called at the one debug step (these are full equations; dumping every
    step would be enormous). Returns None when the store is not materialized.
    """
    rs = getattr(state, 'resolved_subs', None)
    if rs is None:
        return None
    d = _rs_as_dict(rs)
    return [[list(k),
             sorted(([list(i), int(c)] for i, c in v.items()),
                    key=lambda e: e[0])]
            for k, v in d.items()]


def _tc_iraws_json(state):
    """The aux cache's indirect_raws as [[sub_int, op, shift], ...].

    aux_flat = (cached_unique, union_bms, raw_id_to_idx, indirect_raws); the
    4th element is the list of (sub_int, ibp_op, shift) triples that DEFINE
    which indirect candidates exist. Diffing this against a from-scratch build
    is what localizes an incremental-cache loss to specific entries.

    Big (~10k triples at depth), so dumped only at SAILIR_TRUTHCULL_DBG_STEP,
    never under SAILIR_TRUTHCULL_FULL.
    """
    aux = getattr(state, 'aux_flat', None)
    if not aux or len(aux) < 4 or not aux[3]:
        return None
    return [[list(si), int(op), list(sh)] for si, op, sh in aux[3]]
...
def _attach_incremental_aux(child, parent, target, env, target_sector,
    ...
    if parent.aux_flat is None:
        if _TC_AUXDBG:
            print(f'[auxdbg] depth={len(child.path)} tgt={list(target)} '
                  f'PARENT AUX IS None -> no incremental, rebuild on use',
                  flush=True)
        return child  # can't do incremental; will rebuild from scratch on use
    new_resolved_sol = child.resolved_subs[target]
    if _TC_AUXDBG:
        # Phase B of compute_indirect_substituted_incremental_packed short-
        # circuits on `if new_sub_int in sub_cache`. A cached EMPTY list there
        # means the new substitution silently contributes no raws. Inspect the
        # shared cache BEFORE the call so a hit is distinguishable from a real
        # "no raws pass the guard".
        _sc = env._raw_eq_cache.get('_sub_cache')
        _hit = (_sc is not None and tuple(target) in _sc)
        print(f'[auxdbg-pre] depth={len(child.path)} '
              f'sub_cache={"MISSING" if _sc is None else len(_sc)} '
              f'hit_for_target={_hit}'
              + (f' cached_len={len(_sc[tuple(target)])}' if _hit else '')
              + f' raw_eq_cache={len(env._raw_eq_cache)}', flush=True)
        if _TC_AUXDBG_DEEP and len(child.path) in (50, 51, 52):
            # Replicate Phase B's guard loop three ways on the SAME target:
            #   cached  - exactly what Phase B sees (env._raw_eq_cache lookups)
            #   fresh   - recomputed now with the live strip threshold
            #   full    - recomputed with NO strip (what the rebuild path uses)
            # A split between `cached` and `fresh` means a STALE cache entry; a
            # split between `fresh` and `full` means the STRIP is responsible.
            _t = tuple(target)
            _thr = ibp_env.get_raw_strip_threshold()
            _n_cached = _n_fresh = _n_full = 0
            _n_incache = 0
            for _op, _shl in env.shifts.items():
                for _sh in _shl:
                    _seed = tuple(_t[i] - _sh[i]
                                  for i in range(ibp_env.N_INDICES))
                    _k = (_op, _seed)
                    _rc = env._raw_eq_cache.get(_k)
                    if _rc is not None:
                        _n_incache += 1
                        if _t in _rc and _rc[_t] != 0:
                            _n_cached += 1
                    _rf = ibp_env.get_raw_equation(
                        env.ibp_t, env.li_t, _op, _seed, min_w12=_thr)
                    if _t in _rf and _rf[_t] != 0:
                        _n_fresh += 1
                    _ru = ibp_env.get_raw_equation(
                        env.ibp_t, env.li_t, _op, _seed)
                    if _t in _ru and _ru[_t] != 0:
                        _n_full += 1
            print(f'[auxdbg-phaseB] depth={len(child.path)} tgt={list(_t)} '
                  f'thr={_thr} | seeds already in raw cache={_n_incache} '
                  f'| guard passes: cached={_n_cached} fresh={_n_fresh} '
                  f'full={_n_full}', flush=True)
    ...
    if _TC_AUXDBG:
        _pi, _ni = parent.aux_flat[3], new_aux[3]
        _nsub = sum(1 for e in _ni if e[0] == tuple(target))
        _cov = len({e[0] for e in _ni})
        print(f'[auxdbg] depth={len(child.path)} tgt={list(target)} '
              f'iraws {len(_pi)}->{len(_ni)} (+{len(_ni) - len(_pi)})  '
              f'triples_for_this_target={_nsub}  '
              f'subs_covered={_cov}/{len(child.resolved_subs)}'
              f'{"   <== NEW SUB CONTRIBUTED NOTHING" if _nsub == 0 else ""}',
              flush=True)
...
# ===== TRUTH-CLOSURE MINNEW CULL REPLAY (this file only; v8 is untouched) ====
# SAILIR_TRUTH_CLOSURE=<closure.json>  the truth engine's closure library as a
#   state-independent SET of (op, seed), dumped by truth_record_variant.py's
#   SAILIR_DUMP_CLOSURE.
# SAILIR_ACTION_SELECT=truthminnew     per step: enumerate -> rank by minnew ->
#   cut to SAILIR_MAX_ACTIONS -> keep only UNUSED CLOSURE rows -> take the best.
# SAILIR_TRUTHCULL_DUMP=<path>         one JSON line per step: step, target,
#   chosen action, and the FULL expression, for term-by-term diffing against a
#   recorder run of the same integral.
# SAILIR_TRUTHCULL_FULL=1              also dump, AT EVERY STEP, the complete
#   resolved substitution store and the complete enumerated action list. This is
#   what makes a line-by-line whole-state comparison possible: with only one step
#   dumped you can see THAT two runs differ but not WHERE it started, and the
#   first real divergence is usually many steps before the first visible one.
#   Costs a few MB per run; off by default (then subs/valid dump only at
#   SAILIR_TRUTHCULL_DBG_STEP).
_TRUTH_CLOSURE = os.environ.get('SAILIR_TRUTH_CLOSURE')
_CLOSURE_SET = None
if _TRUTH_CLOSURE:
    import json as _cj
    with open(_TRUTH_CLOSURE) as _cf:
        _CLOSURE_SET = {(int(_o), tuple(_sd))
                        for _o, _sd in _cj.load(_cf)['closure']}
    print(f'[closure] loaded {len(_CLOSURE_SET)} rows from {_TRUTH_CLOSURE}',
          flush=True)

# ---------------------------------------------------------------------------
# CORRUPTION (SAILIR_CORRUPT_FRAC / _MIN / _MAX / _SEED)
#
# WHY IT LIVES HERE. The 60-target study that established the upstream-fastmaxw
# cull only ever visited TRUTH states, so nothing established that the cull
# keeps the RECOVERY action inside the top-K once the walk is knocked off the
# truth path. Repeating that study with corruption needs scrambled states in
# THIS walk.
#
# It cannot be done by pre-corrupting the closure file and replaying it: the
# closure is loaded read-only above and the walk never scrambles, so it stays on
# a clean path and simply never needs the recovery rows. Measured three times --
# 174 steps, exactly the uncorrupted count.
#
# The scramble rows are added to _CLOSURE_SET, the SAME pool the truth rows live
# in, so an unscramble action is enumerated, culled and picked by exactly the
# same code as a truth action. That is what makes the study valid: the step
# types are structurally identical, and only the distribution of visited states
# changes.
#
# TEST CRITERION, needing no new instrumentation: the label lookup downstream is
# a next() with NO default, so a correct action pushed outside the culled top-K
# raises StopIteration and kills the run instead of silently mislabelling.
#   completes -> the cull preserves recovery actions
#   dies      -> it does not
# Config PORTED VERBATIM from truth_record_variant.py (the recorder that
# generated corr13), same names, same defaults. OFF by default (0.0): with
# _CFRAC == 0 not a line of the injection block runs, so an uncorrupted walk is
# bit-identical to the existing campaign and the two studies stay comparable.
# SAILIR_RECORD_K -- SEPARATE the ACTING cull from the RECORDED cull.
#
# The walk ACTS on `kept` = scored[:max_actions]. To act UNCAPPED pass a huge K
# (e.g. SAILIR_MAX_ACTIONS=1000000000), NOT 0: only the two `kept = scored[...]`
# lines treat 0 as uncapped. _select_actions does `nv <= max_actions` and
# `argpartition(keys, max_actions)[:max_actions]`, so K=0 selects ZERO actions
# and the walk goes STUCK at step 0 (measured, pilot cluster 1917856).
# Acting uncapped means the walk never stalls merely
# because a closure row was culled -- which is what happens if the acting cull
# is set to 100 directly: `surv` comes from `kept`, so an empty top-100 means
# the walk cannot proceed on that target at all (measured: no closure row
# reaches the top 100 on 84% of 400+ step trajectories).
#
# SAILIR_RECORD_K caps only what is WRITTEN as the training action space. When
# the acting action falls outside that top-K, the STEP IS DISCARDED -- the row
# is not written -- but the walk takes the step anyway and the trajectory
# continues. Per-step discard, never per-trajectory.
#
# 0 / unset => record whatever was acted on (legacy behaviour, no separation).
# SAILIR_MEM_CAP_MB -- DISCARD a target whose truth reduction outgrows memory.
#
# THE POINT IS NOT TO AVOID CRASHING. It is that the corpus should contain what
# classical truth-closure generation can AFFORD, and the integrals it cannot
# afford should be kept as a NAMED HELD-OUT SET -- the targets a trained model
# is later asked to solve. Beam success is self-verifying (the active bucket
# drains or the walk goes STUCK), so those targets can be scored WITHOUT ever
# having a truth reduction for them. That is the whole demonstration: train on
# what is tractable, solve what is not.
#
# Simply dropping them would make a smaller corpus and prove nothing, so the
# discarded target is written to SAILIR_MEM_CAP_LIST with the RSS that broke it.
#
# 0 / unset = no cap (current behaviour). Nothing in this project has come close
# to 32 GB so far: measured peak over 1848 walks was median 0 MB, p90 710 MB,
# max 3,077 MB -- so a cap only bites on a genuinely harder target distribution,
# which is exactly when it is wanted.
_MEM_CAP_MB = int(os.environ.get('SAILIR_MEM_CAP_MB', '0') or 0)
_MEM_CAP_LIST = os.environ.get('SAILIR_MEM_CAP_LIST', '')


class MemoryCapExceeded(Exception):
    """Raised when the walk's RSS passes SAILIR_MEM_CAP_MB.

    NOT an error: the target is deferred to the hard-target set by design.
    """
...
def _mem_cap_check(target, step):
    """Discard this target if it has outgrown the cap. Returns None or raises."""
    if _MEM_CAP_MB <= 0:
        return
    rss = _rss_mb()
    if rss < _MEM_CAP_MB:
        return
    tgt = ','.join(str(int(x)) for x in target) if target is not None else '?'
    print(f'[mem-cap] DISCARD {tgt} at step {step}: rss={rss:.0f}MB '
          f'>= cap {_MEM_CAP_MB}MB -- deferred to the hard-target set',
          flush=True)
    if _MEM_CAP_LIST:
        try:
            os.makedirs(os.path.dirname(_MEM_CAP_LIST) or '.', exist_ok=True)
            # Append-only, one line per discarded target. Concurrent walks each
            # write a single short line; O_APPEND keeps them from interleaving.
            with open(_MEM_CAP_LIST, 'a') as f:
                f.write(f'{tgt}\t{step}\t{rss:.0f}\t{_MEM_CAP_MB}\n')
        except Exception as e:
            print(f'[mem-cap] WARNING could not record to {_MEM_CAP_LIST}: {e}',
                  flush=True)
    raise MemoryCapExceeded(f'{tgt} rss={rss:.0f}MB cap={_MEM_CAP_MB}MB')


_RECORD_K = int(os.environ.get('SAILIR_RECORD_K', '0') or 0)
_REC_SKIPPED = [0]      # steps discarded because the truth fell outside top-K
_REC_WRITTEN = [0]      # steps actually written as training rows

if _RECORD_K > 0:
    # ALWAYS report the counters. A separation that silently never fires looks
    # exactly like a passing run -- that is how the corruption study produced a
    # fake PASS=6 FAIL=0 before anyone checked that scrambling had happened.
    import atexit as _atexit

    @_atexit.register
    def _report_record_k():
        w, s = _REC_WRITTEN[0], _REC_SKIPPED[0]
        t = w + s
        print(f'[record-k] SAILIR_RECORD_K={_RECORD_K}  rows written {w}  '
              f'steps DISCARDED (truth outside top-{_RECORD_K}) {s}  '
              f'of {t} recordable steps'
              + (f'  ({100.0*s/t:.2f}% discarded)' if t else ''),
              flush=True)
...
_n_skipped = [0]
_n_noop = [0]
_n_scr_err = [0]         # scramble() raised -- must be reported, never swallowed
_n_scr_why = [0]
if _CFRAC:
    import random
    _dg = os.path.join(os.path.dirname(_BS7_DIR), 'data-gen')
    if _dg not in sys.path:
        sys.path.insert(0, _dg)
    import generate_multisector_data as _gmc
    _crng = random.Random(_CSEED)
    if _CLOSURE_SET is None:
        raise SystemExit('SAILIR_CORRUPT_FRAC needs SAILIR_TRUTH_CLOSURE: the '
                         'scramble rows are unioned INTO the closure library, '
                         'and without one there is nothing to recover with.')
    print(f'[corrupt] frac={_CFRAC} scramble={_CMIN}-{_CMAX} seed={_CSEED}',
          flush=True)

def _sector_id_int(ts):
    """The sector as an INTEGER BITMASK, which is what generate_multisector_data
    expects. This walk carries it as the mask TUPLE get_sector_mask() returns,
    and passing that straight through raised
        TypeError: unsupported operand type(s) for >>: 'tuple' and 'int'
    from get_sector_mask's own (sector_id >> i) & 1. Since that is how the mask
    is built, packing the bits back is the exact inverse.
    """
    if isinstance(ts, int):
        return ts
    return sum(1 << i for i, b in enumerate(ts) if b)
...
def _corrupt_init():
    """Initialise generate_multisector_data the way the recorder does.

    Its topology-derived state lives in MODULE GLOBALS and importing the module
    does NOT populate them. Uninitialised, find_candidates returns nothing,
    scramble breaks at step 0 and hands back its input unchanged -- which is
    indistinguishable from "the corruption ran and happened to change nothing".
    That produced a PASS=6 over six targets that had measured nothing at all.

    Lazy, so ibp_env.PRIME is already set by the worker (this module is
    imported before the worker configures the prime).
    """
    if _gmc_ready[0]:
        return
    import topo_config as _tcfg
    from sailir.topology import Topology
    _gmc.init_from_topology(Topology.from_dir(_tcfg.TOPO_DIR))
    _gmc.PRIME = ibp_env.PRIME
    _gmc_ready[0] = True
    print(f'[corrupt] generate_multisector_data initialised '
          f'(topo={_tcfg.TOPO_DIR} prime={_gmc.PRIME})', flush=True)
_TC_DBG_STEPS = ({int(x) for x in
                  os.environ['SAILIR_TRUTHCULL_DBG_STEP'].split(',') if x.strip()}
                 if os.environ.get('SAILIR_TRUTHCULL_DBG_STEP') else None)
# Kept as a scalar for the messages that name a single step; the set is what
# the dump gates on, so several steps can be captured in one run.
_TC_DBG_STEP = min(_TC_DBG_STEPS) if _TC_DBG_STEPS else None
_TC_DUMP = (open(os.environ['SAILIR_TRUTHCULL_DUMP'], 'w')
            if os.environ.get('SAILIR_TRUTHCULL_DUMP') else None)
_TC_FULL = os.environ.get('SAILIR_TRUTHCULL_FULL', '0') == '1'
# SAILIR_TRUTHCULL_TRAIN=<path>: emit one fine-tuning sample per step whose
# action space IS the culled top-K the worker will hand the model at inference.
_TC_TRAIN = (open(os.environ['SAILIR_TRUTHCULL_TRAIN'], 'w')
             if os.environ.get('SAILIR_TRUTHCULL_TRAIN') else None)
# Per-step trace of the incremental aux update: how many indirect_raws the new
# substitution contributed, and how many substitutions the cache covers. A step
# where the new sub contributes 0 is a step whose candidates vanish.
_TC_AUXDBG = os.environ.get('SAILIR_TRUTHCULL_AUXDBG', '0') == '1'
# Ranking criterion for the cull. The walk depends on the picks, so states
# diverge between criteria -- the only well-defined comparison is to RUN each
# criterion and compare success at a fixed K, not to probe ranks along one
# trajectory. maxw/sumw are over the relation's ACTIVE TAIL, matching minnew.
_TC_METRIC = os.environ.get('SAILIR_TRUTHCULL_METRIC', 'minnew')
# SAILIR_TRUTHCULL_UNION=minnew,minmaxw -> split the K budget across these
# rankings and union the survivors (they may be complementary).
_TC_UNION = ([m.strip() for m in os.environ['SAILIR_TRUTHCULL_UNION'].split(',')
              if m.strip()]
             if os.environ.get('SAILIR_TRUTHCULL_UNION') else None)
# Heavier: re-run Phase B's guard loop three ways at the steps around the first
# loss, to separate a stale raw cache from the strip.
_TC_AUXDBG_DEEP = os.environ.get('SAILIR_TRUTHCULL_AUXDBG_DEEP', '0') == '1'


def _tc_want_detail(state):
    """Dump the heavy per-step detail (store + action list) on this step?"""
    return _TC_FULL or (_TC_DBG_STEPS is not None
                        and len(state.path) in _TC_DBG_STEPS)

_ACTION_SELECT = os.environ.get('SAILIR_ACTION_SELECT', 'first900')
_METRIC_STRATEGIES = ('maxweight', 'shortest', 'sumweight', 'truthminnew')
# VALIDATE: _select_actions ends in a bare `else: sumweight`, so an unrecognised
# strategy silently ran a DIFFERENT rule than the one asked for.
if _ACTION_SELECT not in _METRIC_STRATEGIES + ('first900', 'last900'):
    raise SystemExit(f'unknown SAILIR_ACTION_SELECT={_ACTION_SELECT!r}')
if _ACTION_SELECT == 'truthminnew' and _CLOSURE_SET is None:
    raise SystemExit('SAILIR_ACTION_SELECT=truthminnew requires '
                     'SAILIR_TRUTH_CLOSURE=<closure.json>')
...
def _truthcull_pick(valid, target, state, env, max_actions, start_w12,
                    target_sector):
    """Cull by minnew, restrict to unused closure rows, return the best one.

    Returns (chosen_or_None, n_pre, n_surv, nv).

    WHY THIS IS NOT DONE INSIDE _select_actions. That function scores from
    `cu`, and DIRECT rows are not in `cu` (EnumCache stores only the indirect
    loop, since direct seeds derive from the target and are never revisited).
    Its metric loop therefore assigns every direct action the sentinel
    -(1<<62) -- correct for its real job, which is guaranteeing the CAP never
    drops a direct, but fatal if the same keys are used to PICK: all directs
    tie at the sentinel and the tie-break becomes enumeration order. Directs
    are ~50% of chosen actions, so half the picks would be arbitrary.

    Here every candidate gets a REAL score. Rows come from `cu` when present
    and are materialised exactly as apply_action_v5 does otherwise, so a direct
    action is scored on the same footing as an indirect one.

    minnew(row) = |{k in row : k != target, in sector, non-master,
                    at-or-above the start in the total order}  MINUS  expr|
    The active-tail filter matches the recorder's
    (sector_of(k)==S0, not master, tkey(k)<=kT); a term already in the
    expression is not new work, so it is subtracted.
    """
    nv = len(valid)
    n_idx = ibp_env.N_INDICES
    used = {(o, tuple(t[i] + d[i] for i in range(n_idx)))
            for (t, o, d) in state.path}
    expr_keys = set(state.expr)

    _store_maxw = _rsd = None
    if _TC_METRIC in ('fastmaxw', 'fastmaxw_minnew', 'upstream'):
        # ONE scalar per substitution: maxw of its (already back-substituted)
        # solution. Computed once per STEP, not per candidate. Measured against
        # the exact resolved maxw on real states: equal 99.9% of the time, never
        # below (cancellations only delete terms), identical top-500 and
        # identical closure retention. See upstream_bound.py.
        _rsd = _rs_as_dict(state.resolved_subs)
        _store_maxw = {k: max((_w12(i) for i in v), default=(0, 0))
                       for k, v in _rsd.items()}

    def _fast_maxw(op, seed):
        """Upper bound on the resolved row's maxw, from the store alone.

        raw_terms = seed+shift (integer arithmetic, no eval_coeff); a term that
        is a store key contributes its sub's maxw, else its own weight. Exact
        except for cancellations, which only remove terms. resolved_subs is
        fully back-substituted, so one pass is final -- no recursion.
        """
        b = (0, 0)
        for sh in env.shifts[op]:
            k = tuple(seed[i] + sh[i] for i in range(n_idx))
            m = _store_maxw.get(k)
            if m is None:
                m = _w12(k)
            if m > b:
    ...
    def _row(op, seed):
        """The resolved relation for (op, seed), exactly as apply_action_v5
        builds it: from cu when present, materialised otherwise (directs are
        never in cu)."""
        if state.aux_flat is not None:
            _i = state.aux_flat[2].get((op, seed))
            if _i is not None:
                return _v7_as_dict(state.aux_flat[0][_i])
        raw = env.get_raw_equation_cached(op, seed)
        return (apply_resolved_subs_dict_x_packed(
                    raw, state.resolved_subs, _V7_REGISTRY, ibp_env.PRIME)
                if _PACKED_RS else
                apply_resolved_subs(raw, state.resolved_subs))

    if _TC_METRIC == 'upstream':
        # FULLY UPSTREAM: admissibility AND ranking from the store alone, so
        # _row() is never called during scoring. A candidate can act on target
        # iff target is one of its raw terms (direct) or target appears in the
        # solution of a store key that is one of its raw terms (indirect) --
        # resolved_subs is fully back-substituted, so one pass is final.
        #
        # This IGNORES coefficients, so it OVER-ADMITS: a raw term whose
        # coefficient is zero, or a target that cancels, is still counted. That
        # is cleaned up at pick time -- the chosen action is materialised and
        # verified, falling through to the next survivor if it cannot act, so
        # the cost is O(1) materialisations per step instead of O(nv).
        scored = []
        for ai in range(nv):
            op, delta = valid[ai]
            seed = tuple(target[i] + delta[i] for i in range(n_idx))
            ...
            for sh in env.shifts[op]:
                ...
                if m > b:
                    b = m
            if adm:
                scored.append(((b, op, tuple(delta)), ai, op, seed))
        scored.sort(key=lambda e: e[0])
        kept = scored[:max_actions] if max_actions and max_actions > 0 else scored
        n_pre = sum(1 for _k, _ai, op, seed in scored
                    if (op, seed) not in used and (op, seed) in _CLOSURE_SET)
        surv = [(k, ai, op, seed) for k, ai, op, seed in kept
                if (op, seed) not in used and (op, seed) in _CLOSURE_SET]
        # VERIFY ON PICK: materialise only the chosen candidate.
        for _si, (_k, ai, op, seed) in enumerate(surv):
            cached = _row(op, seed)
            if target in cached and cached[target] != 0:
                if _TC_TRAIN is not None:
                    # TRAINING SAMPLE, aligned with inference BY CONSTRUCTION.
                    # valid_actions is `kept` -- the top-K after the cull, in
                    # RANK order -- because that is exactly the list the worker
                    # hands the model at inference. Recording the full
                    # enumeration instead would train on a space the model
                    # never sees (and the recorder's own enumeration uses the
                    # LOOSER filter_lateral=False filter, so it is not even the
                    # same set). chosen_action_idx indexes into `kept`.
                    import json as _tj
                    # RECORDED cull, separate from the ACTING cull above.
                    # `kept` is what the walk acted on (uncapped when
                    # max_actions <= 0); `_krec` is what the model will see.
                    _krec = kept[:_RECORD_K] if _RECORD_K > 0 else kept
                    _acts = [[o, list(d)] for (_kk, _aa, o, _ss) in _krec
                             for d in (valid[_aa][1],)]
                    _chosen = [valid[ai][0], list(valid[ai][1])]
                    _ci = next((i for i, (_kk, _aa, _o, _ss) in enumerate(_krec)
                                if _aa == ai), None)
                    if _ci is None:
                        # DISCARD THIS STEP: the acted-on (truth) action fell
                        # outside the recorded top-K, so there is no correct
                        # label inside the action space the model would see.
                        # The walk still TAKES the step -- only the training row
                        # is dropped -- so the trajectory continues intact.
                        _REC_SKIPPED[0] += 1
                        return valid[ai], n_pre, len(surv), nv
                    # multi-label: every unused closure row that SURVIVED the
                    # cull is an equally correct choice reachable at inference.
                    # Positions must index into the RECORDED list (_krec), which
                    # is what `labels` / `valid_label_idxs` refer to downstream.
                    # Indexing into `kept` here would emit label indices beyond
                    # the recorded action space whenever the two differ.
                    _keptpos = {_aa: i for i, (_kk, _aa, _o, _ss)
                                in enumerate(_krec)}
                    _labels = sorted(_keptpos[_aa] for (_kk, _aa, _o, _ss)
                                     in surv if _aa in _keptpos)
                    _REC_WRITTEN[0] += 1
                    _TC_TRAIN.write(_tj.dumps({
                        'source': 'truthcull_upstream_fastmaxw',
                        # Self-describing: a sample is only aligned with a
                        # worker running the SAME cull. Recording it here means
                        # a K=1000 dataset can never be silently trained
                        # against a K=500 worker.
                        # K is the RECORDED cull -- the action space the model
                        # sees, and the one an inference worker must match.
                        # act_K is what the WALK searched (0 = uncapped); they
                        # differ by design when SAILIR_RECORD_K is set.
                        'cull': {'admissibility': 'upstream',
                                 'ranking': 'fastmaxw',
                                 'K': int(_RECORD_K if _RECORD_K > 0
                                          else max_actions),
                                 'act_K': int(max_actions),
                                 'iraws_keep_first': os.environ.get(
                                     'SAILIR_IRAWS_KEEP_FIRST', 'cli')},
                        'step': len(state.path),
                        'target': list(target),
                        'target_weight': list(ibp_env.weight(target)[:2]),
                        'sector_mask': list(target_sector),
                        'expr': sorted(([list(k), int(v)]
                                        for k, v in state.expr.items()),
                                       key=lambda e: e[0]),
                        # REQUIRED by preprocess_to_tensors.load_and_convert
                        # (hard s['subs'] access), then DROPPED by
                        # pack_samples(include_subs=False), which is how the
                        # corr13 shards were built -- verified: they contain no
                        # sub_integrals. Emitting the real store would cost GBs
                        # for data the nosubs model never sees.
                        'subs': [],
                        'valid_actions': _acts,
                        'num_valid_actions': len(_acts),
                        'n_enumerated_pre_cull': nv,
                        'chosen_action': _chosen,
                        'chosen_action_idx': _ci,
                        'valid_label_idxs': _labels,
                        'n_valid_labels': len(_labels),
                    }) + chr(10))
                    _TC_TRAIN.flush()
                return valid[ai], n_pre, len(surv), nv
        return None, n_pre, 0, nv

    # ---- PASS 1: score EVERY candidate by minnew ------------------------
    # The cull ranks the WHOLE enumerated list. Scoring only the closure rows
    # (the previous version) and taking the global best made `max_actions`
    # inert: filtering to the closure BEFORE the cut can never fail, so K had
    # no effect and every K arm trivially agreed. The cut must come first.
    scored = []
    for ai in range(nv):
        op, delta = valid[ai]
        seed = tuple(target[i] + delta[i] for i in range(n_idx))
        cached = _row(op, seed)
        if target not in cached or cached[target] == 0:
            continue          # cannot actually act on the target
        # Active tail of this relation: in-sector, non-master, at-or-above the
        # start. minnew subtracts terms already in the expression (not new work).
        _tail = [k for k in get_non_masters(cached, target_sector)
                 if k != target and is_active(k, start_w12)]
        n_new = sum(1 for k in _tail if k not in expr_keys)
        if _TC_UNION:
            _w = [ibp_env.weight(k)[:2] for k in _tail]
            _maxw = max(_w) if _w else (0, 0)
            _new = [k for k in _tail if k not in expr_keys]
            _newsumw = (sum(ibp_env.weight(k)[0] for k in _new),
                        sum(ibp_env.weight(k)[1] for k in _new))
            _by = {'minnew': n_new, 'minmaxw': _maxw, 'minnewsumw': _newsumw,
                   'minsumw': (sum(a for a, _b in _w), sum(b for _a, b in _w))}
            _key = tuple(_by[m] for m in _TC_UNION)
            scored.append((_key, ai, op, seed))
            continue
        if _TC_METRIC == 'minnew':
            _key = (n_new,)
        else:
            _w = [ibp_env.weight(k)[:2] for k in _tail]
            _maxw = max(_w) if _w else (0, 0)
            _sumw = (sum(a for a, _b in _w), sum(b for _a, b in _w))
            _new = [k for k in _tail if k not in expr_keys]
            _newsumw = (sum(ibp_env.weight(k)[0] for k in _new),
                        sum(ibp_env.weight(k)[1] for k in _new))
            if _TC_METRIC == 'fastmaxw':
                _key = (_fast_maxw(op, seed),)
            elif _TC_METRIC == 'fastmaxw_minnew':
                _key = (_fast_maxw(op, seed), n_new)
            elif _TC_METRIC == 'minnewsumw':
                # sumw analogue of minnew: total weight of the NEW terms only,
                # rather than their count. In the recorder's _CANON list.
                _key = (_newsumw,)
            elif _TC_METRIC == 'newsumw_maxw':
                _key = (_newsumw, _maxw)
            elif _TC_METRIC == 'maxw_newsumw':
                _key = (_maxw, _newsumw)
            elif _TC_METRIC == 'minmaxw':
                _key = (_maxw,)
            elif _TC_METRIC == 'minnew_maxw':      # minnew first, maxw breaks ties
                _key = (n_new, _maxw)
            elif _TC_METRIC == 'maxw_minnew':      # maxw first, minnew breaks ties
                _key = (_maxw, n_new)
            elif _TC_METRIC == 'minsumw':
                _key = (_sumw,)
            elif _TC_METRIC == 'maxw_new_sumw':
                _key = (_maxw, n_new, _sumw)
            else:
                raise SystemExit(f'unknown SAILIR_TRUTHCULL_METRIC={_TC_METRIC}')
        scored.append((_key + (op, tuple(delta)), ai, op, seed))

    # ---- CUT to the top max_actions -------------------------------------
    if _TC_UNION:
        # Complementary-criteria mode: split the K budget across several
        # rankings and take the UNION, so a closure row that is mediocre under
        # one criterion still survives if it ranks high under another.
        per = max(1, max_actions // len(_TC_UNION)) if max_actions else len(scored)
        picked, seen_ai = [], set()
        for mi in range(len(_TC_UNION)):
            for e in sorted(scored, key=lambda e, _m=mi: e[0][_m])[:per]:
                if e[1] not in seen_ai:
                    seen_ai.add(e[1])
                    picked.append(e)
        kept = picked
    else:
    ...
    # ---- THEN keep only UNUSED CLOSURE rows -----------------------------
    # n_pre  = unused closure rows present BEFORE the cut (what was available)
    # n_surv = unused closure rows that SURVIVED the cut (what K left us)
    n_pre = sum(1 for _k, _ai, op, seed in scored
                if (op, seed) not in used and (op, seed) in _CLOSURE_SET)
    surv = [(k, ai) for k, ai, op, seed in kept
            if (op, seed) not in used and (op, seed) in _CLOSURE_SET]
    if _TC_DBG_STEP is not None and len(state.path) == _TC_DBG_STEP:
        for k, _ai, op, seed in kept[:15]:
            print(f'   [dbg] rank op={op} minnew={k[0]} '
                  f'closure={(op, seed) in _CLOSURE_SET} '
                  f'used={(op, seed) in used}', flush=True)
    if not surv:
        # DIAGNOSTIC on death: where do the closure rows actually RANK, and is
        # the cut arbitrary? A minnew tie-break of (op, delta) means that if
        # many candidates share the minimum n_new, the top-K is filled by lowest
        # op number -- which has nothing to do with quality and could exclude
        # closure rows systematically. Report the best closure rank and the tie
        # structure so K can be set from evidence.
        _cl_ranks = [r for r, (_k, _ai, op, seed) in enumerate(scored)
                     if (op, seed) not in used and (op, seed) in _CLOSURE_SET]
        _best = _cl_ranks[0] if _cl_ranks else None
        _cut_nn = kept[-1][0][0] if kept else None
        _n_tied = sum(1 for k, _a, _o, _s in scored if k[0] == _cut_nn)
        _cl_nn = scored[_best][0][0] if _best is not None else None
        print(f'   [DEAD] nv={nv} K={max_actions} closure_avail={n_pre} '
              f'best_closure_rank={_best} its_minnew={_cl_nn} '
              f'cut_at_minnew={_cut_nn} n_tied_at_cut={_n_tied} '
              f'closure_ranks={_cl_ranks[:8]}', flush=True)
        return None, n_pre, 0, nv
    surv.sort(key=lambda e: e[0])
    return valid[surv[0][1]], n_pre, len(surv), nv
...
def beam_search_v5(env, model, start_expr, target_sector, start_w12,
    ...
    def _enum_slice(parent_indices, beam):
        ...
        for parent_idx in parent_indices:
            ...
            if _TC_UPENUM_REPLACE:
                # cu is never built: the store-only rule supplies the actions.
                indirect_cache = None
            ...
            for target in tied:
                ...
                _tc_upenum_check(s, target, valid, env)
                ...
                if valid:
                    if _ACTION_SELECT == 'truthminnew':
                        _ch, _npre, _nsurv, _nv = _truthcull_pick(
                            valid, target, s, env, max_actions, start_w12, target_sector)
                        if _TC_DUMP is not None:
                            import json as _tj
                            _TC_DUMP.write(_tj.dumps({
                                'step': len(s.path), 'target': list(target),
                                'action': None if _ch is None else [_ch[0], list(_ch[1])],
                                'nv': _nv, 'closure_pre': _npre,
                                'expr': sorted(([list(k), int(v)]
                                                for k, v in s.expr.items()),
                                               key=lambda e: e[0]),
                                'subs': (_tc_subs_json(s)
                                         if _tc_want_detail(s) else None),
                                'valid': ([[a[0], list(a[1])] for a in valid]
                                          if _tc_want_detail(s) else None),
                                'iraws': (_tc_iraws_json(s)
                                          if (_TC_DBG_STEPS is not None
                                              and len(s.path) in _TC_DBG_STEPS)
                                          else None)})
                                + chr(10))
                            _TC_DUMP.flush()
                        print(f'[truthcull] nv={_nv} K={max_actions} '
                              f'closure_pre={_npre} surv={_nsurv} '
                              f'tgt={list(target)}'
                              f'{"" if _ch else " DEAD"}', flush=True)
                        valid = [] if _ch is None else [_ch]
    ...
    for step in range(resume_step, max_steps):
        # MEMORY CAP: discard this target rather than let the truth reduction
        # grow without bound. Checked at the TOP of the step so the breach is
        # attributed to the step that caused it, and so a resumed run re-checks
        # immediately rather than after doing more work.
        # start_expr = {start_int: 1} from the worker, so its single key is
        # the launch integral -- the right identity for a DISCARDED TARGET.
        # `target` is not bound until inside the per-state work below.
        _mem_cap_check(next(iter(start_expr)) if len(start_expr) == 1
                       else None, step)
        ...
        # ---- INJECT A CORRUPTION ----------------------------------------
        # PORTED from truth_record_variant.py's injection block. Only the
        # containers differ: the recorder appends scramble rows to used_ibps /
        # cached_rows / occur, and this walk's equivalent pool is _CLOSURE_SET,
        # so the rows are unioned into that. The effect is the same and it is
        # the whole point -- at a scrambled state the candidate pool holds BOTH
        # unused truth-closure rows AND the unscramble row, both enumerated,
        # both ranked by the same cull, indistinguishable downstream.
        #
        # NO GUARD ON WHAT THE SCRAMBLE INTRODUCES (the recorder's finding):
        # re-introducing a solved integral is handled by folding the store back
        # through the scrambled expression below, and reaching subsectors or
        # lighter integrals is normal -- they are stripped as passengers. Both
        # old guards cost 30 of 49 draws and left 1 in 50 usable. A draw is
        # skipped only if it changes nothing after the fold.
        #
        # resolved_subs is untouched, so aux_flat stays valid; only the derived
        # expr weights are recomputed.
        if _CFRAC and _crng.random() < _CFRAC:
            _corrupt_init()
            for _bi, _st in enumerate(beam):
                _n_attempt[0] += 1
                for _try in range(6):
                    _ns = _crng.randint(_CMIN, _CMAX)
                    # REPRODUCIBILITY. gm.scramble draws from the GLOBAL random
                    # module and nothing seeds it, so the corruption differed
                    # run to run (86 vs 84 steps on identical configs). Seed per
                    # (step, beam slot, retry) so a rebuild reproduces exactly.
                    random.seed((_CSEED * 1000003 + step * 9176
                                 + _bi * 131 + _try) & 0x7fffffff)
                    try:
                        _sc, _new = _gmc.scramble(
                            _st.expr, env.ibp_t, env.li_t, len(env.shifts),
                            _ns, _sector_id_int(target_sector),
                            filter_lateral=False)
                    except Exception as _e:
                        # The recorder swallows this silently. Here it must NOT
                        # be silent: a swallowed exception looked exactly like
                        # "corruption ran and changed nothing", and produced a
                        # PASS=6 that had measured nothing at all.
                        if _n_scr_err[0] == 0:
                            import traceback
                            print(f'[corrupt] scramble RAISED: {_e!r}',
                                  flush=True)
                            traceback.print_exc()
                        _n_scr_err[0] += 1
                        _sc, _new = None, []
                    if not _new or _sc is None or _sc == _st.expr:
                        if _n_scr_err[0] == 0 and _n_scr_why[0] == 0:
                            print(f'[corrupt] draw rejected: new={len(_new)} '
                                  f'sc_is_none={_sc is None} '
                                  f'unchanged={_sc == _st.expr}', flush=True)
                            _n_scr_why[0] += 1
                        continue
                    # RESTORE THE INVARIANT: expr is always reduced w.r.t. the
                    # store, and scramble() breaks that by injecting integrals
                    # without folding the store through the result.
                    _ne = strip_passenger(
                        ibp_env.apply_all_substitutions(
                            dict(_sc), _rs_as_dict(_st.resolved_subs)),
                        start_w12)
                    if _ne == _st.expr:
                        _n_noop[0] += 1
                        continue
                    _nm = get_non_masters(_ne, target_sector)
                    _mw, _tw = _mw_tw_from_nm(_nm)
                    beam[_bi] = _st._replace(
                        expr=_ne, n_non_masters=len(_nm),
                        max_w12=_mw, total_w12=_tw)
                    _CLOSURE_SET.update((int(_o), tuple(_sd))
                                        for _o, _sd in _new)
                    _n_injected[0] += len(_new)
                    _n_ok[0] += 1
                    break
                else:
                    _n_skipped[0] += 1
            if verbose:
                print(f'[corrupt step {step}] ok={_n_ok[0]} '
                      f'skipped={_n_skipped[0]} noop={_n_noop[0]} '
                      f'injected={_n_injected[0]} '
                      f'closure={len(_CLOSURE_SET)}', flush=True)
            ...
            for parent_idx, s in enumerate(beam):
                ...
                for target in tied:
                    ...
                    #    re-allows the highest-prob overflow using model probs.
                        ...
                        _bt_key = (frozenset(s.expr.items()), target)
                        _bt_set = _bt_tabu.get(_bt_key)
                        # FULL TABU INSTRUMENTATION at one chosen step. Answers,
                        # for every closure row the tabu removes: who banned it,
                        # at what path length, and whether that writer is an
                        # ANCESTOR of this lineage (a genuine loop) or an
                        # unrelated state that merely shared this expression
                        # (the tabu key is the EXPRESSION, but a state is
                        # (expr, subs)).
                        if (_TC_DBG_STEP is not None
                                and len(s.path) == _TC_DBG_STEP):
                            _mp = tuple(s.path)
                            print(f'   [tabu-dbg] step={len(s.path)} '
                                  f'valid_before={len(valid)} '
                                  f'bt_set={0 if not _bt_set else len(_bt_set)} '
                                  f'tabu_cap_eff={_tabu_cap_eff} '
                                  f'expr_terms={len(s.expr)}', flush=True)
                            for _a in (_bt_set or ()):
                                if _a not in valid:
                                    continue
                                _seed_a = tuple(target[_i] + _a[1][_i]
                                                for _i in range(
                                                    ibp_env.N_INDICES))
                                _inc = (_CLOSURE_SET is not None
                                        and (_a[0], _seed_a) in _CLOSURE_SET)
                                _w = _BT_WHO.get((_bt_key[0], target, _a)) or ()
                                _isanc = any(
                                    _L <= len(_mp) and hash(_mp[:_L]) == _H
                                    for _L, _H in _w)
                                print(f'      BANNED op={_a[0]} '
                                      f'delta={list(_a[1])} '
                                      f'in_closure={_inc} '
                                      f'n_writers={len(_w)} '
                                      f'writer_depths='
                                      f'{sorted(_L for _L, _ in _w)} '
                                      f'ancestor_of_me={_isanc}', flush=True)
                        if _bt_set:
                            _nblk = sum(1 for a in valid if a in _bt_set)
                            if _nblk < len(valid) and _nblk <= _tabu_cap_eff:
                                _pre_bt = list(valid)
        ...
        # _model_free_row OVERWRITES every row below when _ACTION_SCORE !=
        # 'model', so the forward was pure waste; with model=None it also
        # crashes. probs_all is already zeros, which the overwrite expects.
        _skip_model = (_ACTION_SCORE != 'model') or (model is None)
