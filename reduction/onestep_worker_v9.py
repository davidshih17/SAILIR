...
=================================================================
Hierarchical-reduction worker that runs `beam_search_v9.beam_search_v5`
IN-PROCESS — exactly the way onestep_worker_v6.py runs v6, and exactly the way
the verified probes ran `python beam_search_v9.py --n-threads 8 --n-workers 8`.

WHY IN-PROCESS (and not a subprocess wrapper):
  A single process means beam_search_v9's import-time _cap_incidental_threads()
...
v7 SETTINGS reproduced (== the success-only probe recipe):
  ENV (set BELOW, before importing beam_search_v9, because _SUCCESS_TOTAL /
...
# ── v7 toggles: MUST be set before importing beam_search_v9 (module-level
...
# ── argv injection so beam_search_v9's import-time _cap_incidental_threads()
...
# beam_search_v9, because its import-time _cap_incidental_threads() reads the
...
# symmetry-first block below imports sailir/numpy ahead of beam_search_v9's
...
# Before paying the beam_search_v9/torch import + model load, try the symmetry
...
# Import beam_search_v9 FIRST. Its module top runs _cap_incidental_threads()
...
# exceeds RequestCpus -> Condor holds the job. So beam_search_v9 (hence its
# torch) must load before any other torch-importing module. This mirrors the
# standalone beam_search_v9.py order (_cap_incidental_threads() then import torch).
import beam_search_v9 as bs7
from beam_search_v9 import (beam_search_v5, replay_full_expr, _is_success,
                            _target_key, max_w12)
import torch
...
def main():
    ...
    # ── replicate beam_search_v9.main()'s setup VERBATIM ──────────────────
    ...
    # v7 module globals that beam_search_v9.main() sets (replicate exactly).
    ...
    if os.environ.get('SAILIR_SYM_DROP', '0') == '1':
        ...
        # See the banner in beam_search_v9.py. Kept for retrain-era experiments.
    ...
    # beam_search_v9.main() does for its --output.
