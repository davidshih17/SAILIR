#!/usr/bin/env python
"""CORRECTED replay: uses secbits() over range(ND)=8. Classify dispatched (reduced) and
passenger integrals as asymmetric / symmetric-survivor / symmetric-reducible-FREE."""
...
P=1009; ND=8
def secbits(i): return tuple(1 if i[k]>0 else 0 for k in range(ND))     # 8-tuple, CORRECT
...
def rref(rows,w):
    ...
    return rows[:r]
def classify(I):
    corner=corner_of(sector(I)); autos=sym_autos(corner)
    nt=[t for t in autos if image_unsigned(corner,*t)!={corner:1}]
    if not nt: return "asymmetric"
    rels=[]; ints={I}
    for t in autos:
...
print("SANITY sector_symmetric (want True,True,True):")
for s in [(1,2,4,7),(0,3,4,5),(0,1,2,3)]:
    corner=corner_of(s); nt=[t for t in sym_autos(corner) if image_unsigned(corner,*t)!={corner:1}]
    print(f"  {s}: {len(nt)} nontrivial autos")
...
report("passengers (next-level targets)", expr-subs)
