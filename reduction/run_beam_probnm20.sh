...
export SAILIR_BEAM_NM_LAMBDA=0.03   # cost = lambda*nm - log(prob)    # keys topo_config; default pentagonbox would assert
...
nohup python -u reduction/beam_search_v7.py \
    ...
    --beam-sort   prob_nm \
    --max-steps   2000 \
    --output      results/beam_probnm20_bce.pkl \
    > logs/beam_probnm20_bce.log 2>&1 &
echo $! > logs/beam_probnm20_bce.pid
echo "Started PID=$(cat logs/beam_probnm20_bce.pid). Tail: tail -f logs/beam_probnm20_bce.log"
