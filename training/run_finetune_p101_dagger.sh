...
#
#   --resume checkpoints/gravity3L_p101_bce/best_model.pt
...
mkdir -p checkpoints/gravity3L_p101_dagger logs
export CUDA_VISIBLE_DEVICES=0
export PYTHONUNBUFFERED=1
export SAILIR_LOSS=bce
LOG=logs/train_p101_dagger.log
METRICS=logs/train_p101_dagger_metrics.tsv
...
echo "shards_dir=data_combined (788 base + 64 dagger)" | tee -a "$LOG"
nohup python -u training/train_classifier.py \
    --topology         topology_input/gravity3L \
    --shards_dir       data_combined \
    --resume           checkpoints/gravity3L_p101_bce/best_model.pt \
    --restart_lr       1e-5 \
    --output_dir       checkpoints/gravity3L_p101_dagger \
    --log_file         "$METRICS" \
    ...
    >> "$LOG" 2>&1 &
PID=$!
echo "$PID" > logs/train_p101_dagger.pid
